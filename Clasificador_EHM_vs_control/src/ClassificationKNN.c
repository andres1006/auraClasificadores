...
void ClassificationKNN_score(const double obj_X[112], const unsigned int
  ...
  emxArray_boolean_T *nonnanscoreindices)
{
  ...
  if (numNeighbors > 0) {
    ...
    for (i = 0; i < 10; i++) {
      if (nonnanIndices[i]) {
        for (vstride = 0; vstride < nx; vstride++) {
          if (rtIsNaN(dist->data[vstride + dist->size[0] * i])) {
            distWgt->data[vstride + distWgt->size[0] * i] = 0;
          }
        }
      }
    }
    ...
    for (vstride = 0; vstride < acoef; vstride++) {
      ...
      wgtval = (double)distWgt->data[vstride] * 0.035714285714285712;
      wgt->data[sck] = wgtval;
    }
