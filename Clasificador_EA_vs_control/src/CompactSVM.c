...
void CompactSVM_normalize(const double obj_Mu[12], const double obj_Sigma[12],
  emxArray_real_T *X)
{
  emxArray_real_T *a;
  int i0;
  ...
  emxFree_real_T(&a);
  bcoef = 0;
  for (csz_idx_0 = 0; csz_idx_0 < 12; csz_idx_0++) {
  ...
  for (csz_idx_0 = 0; csz_idx_0 < 12; csz_idx_0++) {
    if (obj_Sigma[csz_idx_0] > 0.0) {
      tmp_data[b_acoef] = (signed char)(csz_idx_0 + 1);
      b_acoef++;
    }
  }
  ...
  for (csz_idx_0 = 0; csz_idx_0 < 12; csz_idx_0++) {
    if (obj_Sigma[csz_idx_0] > 0.0) {
      b_tmp_data[b_acoef] = (signed char)(csz_idx_0 + 1);
      b_acoef++;
    }
  }
